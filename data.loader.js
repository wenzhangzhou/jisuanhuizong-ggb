(function(){
  ...
  function loadParts(){
    return Promise.all(PARTS.map(function(k){
      ...
      var finish=function(){ if(!d.geometry) d.geometry=[]; boot(d); };
      if(d.geometry && d.geometry.length){ finish(); return; }
      return loadGeometrySplit().then(function(g){ d.geometry=g; finish(); });
    });
  }
  Promise.all([

...
启用方式（若尚未开启）：仓库 **Settings → Pages → Build and deployment → Source** 选 **Deploy from a branch**，Branch 选 `main`，Folder 选 `/ (root)`，Save。本仓库根目录已含静态站文件与 `.nojekyll`，无需构建。也可选 **GitHub Actions** 若已启用 `.github/workflows/static.yml`。

## 本地打开

```bash
cd screw-selector-web   # 或本仓库根目录
